...
- (NSString*)systemUserAgent
{
    ...
    NSString *kmL_Jira_TestUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
    return [NSString stringWithFormat:@"%@/%s (%@)",kmL_Jira_TestUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo];
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a mL_Jira_Test app using the node.js CLI
