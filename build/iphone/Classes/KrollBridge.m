...
NSString * mL_Jira_Test$ModuleRequireFormat = @"(function(exports){"
...
@implementation mL_Jira_TestObject
...
{
	// create mL_Jira_Test global object
	NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];

	// Load the "mL_Jira_Test" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ml_jira_test = [[mL_Jira_TestObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ml_jira_test];

	NSString *_ml_jira_testNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ml_jira_testNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ml_jira_test valueForKey:name];
...
-(KrollWrapper *)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:mL_Jira_Test$ModuleRequireFormat, dirname, filename, code];
...
- (KrollWrapper *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
	...
	if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
		@throw [NSException exceptionWithName:@"org.ml_jira_test.kroll"
